...
void tests_b_g_fixed(char* instance_file, float* solvingTime1D, float* solvingTime2D){
	...
	time_t end;
	for (int n=0; n<10; n++){
		modify_instance_params(instance_file, (n+1)*10, b, g); 
...
void tests_n_b_fixed(char* instance_file, float* solvingTime1D, float* solvingTime2D){
	...
	time_t end;
	for (int g=0; g<10; g++){
		modify_instance_params(instance_file, n, b, 100*(g+1)); 
		//----------
		dataSet data;
		FILE* fin = fopen(instance_file,"r");
		read_TP2_instance(fin,&data);
    	...
    	secondes = (unsigned long) difftime( end, begin );
		solvingTime1D[g] = secondes;
		
		begin = time( NULL );
		solve_2DKP(&data);
		end = time( NULL);
		secondes = (unsigned long) difftime( end, begin );
		solvingTime2D[g] = secondes;
	}
}
...
void tests_n_g_fixed(char* instance_file, float* solvingTime1D, float* solvingTime2D){
	...
	time_t end;
	for (int b=0; b<10; b++){
		modify_instance_params(instance_file, n, 100*(b+1), g); 
		//----------
		dataSet data;
		FILE* fin = fopen(instance_file,"r");
		read_TP2_instance(fin,&data);
    	...
    	secondes = (unsigned long) difftime( end, begin );
		solvingTime1D[b] = secondes;
		
		begin = time( NULL );
		solve_2DKP(&data);
		end = time( NULL);
		secondes = (unsigned long) difftime( end, begin );
		solvingTime2D[b] = secondes;
	}
}
